...
function calculateLearnerResult(rec){
  ...
  assessment.items.forEach(it=>{ total+=it.points; score+=scoreAnswer(it,merged[it.no]); if(['NUMERICAL','ALGEBRAIC'].includes(it.type)&&!String(merged[it.no]||'').trim())pending++; });
